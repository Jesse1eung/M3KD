...
class LengthDropTrainer(Trainer):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if hasattr(self.model, "module"):
                    model_config_numhiddenlayers = self.model.module.config.num_hidden_layers
                else:
                    model_config_numhiddenlayers = self.model.config.num_hidden_layers
    ...
    def evaluate(self, eval_dataset=None,
        ...
        output, timecost = self.prediction_loop(eval_dataloader, description="Evaluation",
        ...
        return output.metrics, timecost
    ...
    def prediction_loop(
        ...
        # print('### TOTAL TIME COST: %f ###' % timecost)
        ...
        return PredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics), timecost
    ...
    def prediction_step(
        ...
        if length_config is not None:
            ...
            inputs["output_attentions"] = True
            length_config_new = []
            for _ in length_config:
                length_config_new.append(_)
            inputs["length_config"] = tuple(length_config_new)

        inputs = self._prepare_inputs(inputs)
        if hasattr(model, "module"):
            model_config_outputattentions = model.module.config.output_attentions
            model_config_outputhiddenstates = model.module.config.output_hidden_states
        else:
            model_config_outputattentions = model.config.output_attentions
            model_config_outputhiddenstates = model.config.output_hidden_states

        output_attentions = output_attentions if output_attentions is not None else model_config_outputattentions
        output_hidden_states = output_hidden_states if output_hidden_states is not None else model_config_outputhiddenstates
    ...
    def init_evolution(self, lower_constraint=0, upper_constraint=None):
        ...
        if hasattr(self.model, "module"):
            model_config_modeltype = self.model.module.config.model_type
        else:
            model_config_modeltype = self.model.config.model_type

        if model_config_modeltype == "distilbert":
    ...
    def add_gene(self, gene, macs=None, score=None, method=0, parents=None):
        if gene not in self.store:
            ...
            if model_config_modeltype == "distilbert":
                ...
                model_config_modeltype = 'distilbert'
            elif model_config_modeltype == "bert":
                assert hasattr(self.model, "bert")
                bert = self.model.bert
                model_config_modeltype = 'bert'
            else:
                bert = self.model.roberta
                model_config_modeltype = 'roberta'
            ...
            score = score or self.evaluate(model_type=model_config_modeltype,
                                           length_config=gene)[0]["eval_" + self.best_metric]
    ...
    def mutate(self, mutation_prob):
        ...
        for i in range(model_config_numhiddenlayers):
            if np.random.uniform() < mutation_prob:
                prev = (self.args.max_seq_length if i == 0 else mutated_gene[i - 1])
                next = (2 if i == model_config_numhiddenlayers - 1 else gene[i + 1])
                mutated_gene += (random.randrange(next, prev + 1),)
            else:
                mutated_gene += (gene[i],)
        mutated_gene = revise_gene(mutated_gene)
    ...
    def crossover(self):
        ...
        crossovered_gene = revise_gene(crossovered_gene)
