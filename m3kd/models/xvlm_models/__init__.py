...
from lavis.models.swt import SwinTransformer, interpolate_relative_pos_embed
from lavis.models.med import BertConfig, BertForMaskedLM, BertModel
from lavis.models.xroberta import RobertaConfig, RobertaForMaskedLM, RobertaModel
...
def build_vision_encoder(config, load_params=False):
    ...
    if config['use_swin']:
        ...
        vision_config = config['swin']
        vision_config['image_res'] = config['image_res']
...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    if config_text is None:
        config['text_config'] = './lavis/configs/models/med_config.json'
...
class XVLMBase(nn.Module):
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat,
                          text_embeds, text_atts, text_feat, idx=None,
        ...
        image_atts_neg = []
        for b in range(bs):
            if image_neg_ids is not None:
                neg_idx = image_neg_ids[b]
            else:
