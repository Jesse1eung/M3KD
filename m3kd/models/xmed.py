...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        try:
            ...
            # self.add_moe = ((fusion_layer -1) <= layer_num and self.config.add_moe)
            # self.add_moe = ((fusion_layer) <= layer_num and self.config.add_moe)
            self.add_moe = ((fusion_layer+2) <= layer_num and self.config.add_moe)
            # self.add_moe = ((layer_num+1)%3 ==0 and self.config.add_moe)

            # print("add_cross_attention", add_cross_attention, fusion_layer, layer_num)
    ...
    def forward(
        ...
        # compatibility for ALBEF and BLIP
        # if mode in ["fusion"]:
        #     print(hasattr(self, "crossattention"),self.layer_num)
        ...
        layer_output = apply_chunking_to_forward(
            self.l_feed_forward_chunk if mode=='mlmnovisual' and self.add_moe else self.feed_forward_chunk,
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        print("add moe: ",self.layer[-1].add_moe)
    ...
    def forward(
        ...
        for i in range(start_layer, output_layer):
            ...
            if mode == "fusion" and output_attentions and self.config.add_cross_attention:
                # print(len(layer_outputs))
                # for k in layer_outputs:
                #     print('f',k.shape if hasattr(k,'shape') else len(k))
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config, add_fit=False):
        super().__init__(config)

        self.bert = BertModel(config, add_pooling_layer=False)
        self.cls = BertOnlyMLMHead(config)
        if add_fit:
            # self.fit_denses = nn.ModuleList(nn.Linear(config.hidden_size, 768) for i in range(config.num_hidden_layers+1))
            self.fit_denses = nn.ModuleList(nn.Linear(1024, 768) for i in range(config.num_hidden_layers+1))
    ...
    def forward(
        ...
        if hasattr(self, "fit_denses"):
            outputs_hid_states = ()
            for i, hid in enumerate(ouputs.hidden_states):
                hid_fit = self.fit_denses[i](hid)
                outputs_hid_states += (hid_fit,)

        return MaskedLMOutput(
            loss=masked_lm_loss,
            logits=prediction_scores,
            hidden_states=outputs.hidden_states if not hasattr(self, "fit_denses") else outputs_hid_states,
