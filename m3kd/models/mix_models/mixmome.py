...
class BeitEncoder(nn.Module):
    ...
    def forward(
        ...
        try:
            ...
            share_layer = self.config.share_layer

        except AttributeError:
            # BLIP
            fusion_layer = self.config.num_hidden_layers
            share_layer = self.config.num_hidden_layers // 2

        if mode == "text":
            start_layer = share_layer if share_layer > 0 else 3
            # output_layer = self.config.fusion_layer
            output_layer = fusion_layer
            # output_layer = self.config.num_hidden_layers #0-2-bert 3-5-beit text
        elif mode == "fusion":
            start_layer = fusion_layer
            start_layer = share_layer #也就是从3开始 fusion
            output_layer = self.config.num_hidden_layers

        elif mode == 'image':
            start_layer = 0
            output_layer = fusion_layer if fusion_layer < self.config.num_hidden_layers else self.config.num_hidden_layers
            # output_layer = self.config.num_hidden_layers # 0-5 image
...
class BeitModel(BeitPreTrainedModel):
    ...
    def load_pretrained(self, ckpt_rpath, load_beit_by_sep=False):
        state_dict = torch.load(ckpt_rpath,)
        state_dict = state_dict['model'] if 'model' in state_dict.keys() else state_dict
        if load_beit_by_sep:
            state_dict_i = {}
            a = set()
            assert 12 % self.config.num_hidden_layers == 0, "student model beit's layers should divide 12"
            for k, v in state_dict.items():
                layer = re.findall(r'\d+' ,k)
                key = k if 'beit' not in k else k[5:]
                if len(layer) == 0:
                    state_dict_i[key] = state_dict[k]
                elif (int(layer[0])+1) % (12//self.config.num_hidden_layers) == 0:
                    m = 12//self.config.num_hidden_layers
                    a.add(layer[0])
                    key = re.sub(r'\d+', str((int(layer[0])+1)// m - 1),key, count=1)
                    state_dict_i[key] = state_dict[k]
                # else:
                #     print(k)
            state_dict = state_dict_i
        msg = self.load_state_dict(state_dict, strict=False)
        print('load bcheckpoint for beit from %s' % ckpt_rpath)
    ...
    def forward(
        ...
        image_embeds  = self.embeddings(pixel_values, pixel_mask,bool_masked_pos)
        # print(image_embeds.shape)
        image_masks = torch.ones(image_embeds.size()[:2]).to(device)
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(image_masks, (image_batch_size,image_embeds.shape[1]), device)
