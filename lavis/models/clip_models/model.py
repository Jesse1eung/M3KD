...
class CLIP(BaseModel):
    ...
    def predict(self, samples):
        ...
        print(image_features.shape, ' 2')
        image_features = F.normalize(image_features, dim=-1)
        print('dim of two: ', image_features.shape, self.classifier.shape)
    ...
    def zero_shot_classifier(self, classnames, templates):
        with torch.no_grad():
            ...
            for classname in classnames:
                print(len(classname),classname,'dsf')
                texts = [
                ...
                print(class_embeddings.shape, ' 1')
                class_embedding = F.normalize(class_embeddings, dim=-1).mean(dim=0)
                print(class_embedding.shape, ' 3')
                class_embedding /= class_embedding.norm()
                zeroshot_weights.append(class_embedding)
            zeroshot_weights = torch.stack(zeroshot_weights, dim=1).to(self.device)
            # print('dim of ze', zeroshot_weights.shape)
