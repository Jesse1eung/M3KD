...
class Attention(nn.Module):
    ...
    def forward(self, x, register_hook=False, return_relation=False):
        B, N, C = x.shape
        qkv = (
            self.qkv(x)
...
class Block(nn.Module):
    def __init__(
        ...
        if self.use_adapter:
            self.att_adapter = BottleneckAdapter(dim,)
            self.mlp_adapter = BottleneckAdapter(dim,)
        if use_grad_checkpointing:
            self.attn = checkpoint_wrapper(self.attn)
            self.mlp = checkpoint_wrapper(self.mlp)

    def forward(self, x, register_hook=False, return_relation=False, use_moe=False):
        if return_relation:
            qk, vv =self.attn(self.norm1(x), return_relation=True)
            return qk, vv
        if self.use_adapter:
            x = self.att_adapter(x)
            x = x + self.drop_path(self.attn(self.norm1(x), register_hook=register_hook))
            x = self.mlp_adapter(x)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.attn(self.norm1(x), register_hook=register_hook))
            if hasattr(self, "v_mlp") and use_moe:
                x = x + self.drop_path(self.v_mlp(self.norm2(x)))
            else:
                x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        print('depth of vit:', depth)
        print('use_adapter of vit:', use_adapter)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    use_adapter=use_adapter and (i+2)%3==0,
                    use_moe=(i==(depth-1) and use_moe),
                )
        ...
        self.fit_denses=nn.ModuleList(nn.Linear(embed_dim, 768) for i in range(depth+1))
    ...
    def forward(self, x, register_blk=-1, return_relation=False,use_moe=False):
        B = x.shape[0]
        ...
        for i, blk in enumerate(self.blocks):
            if i+1 == self.depth and return_relation:
                # print(return_relation,'return relation')
                qk ,vv =blk(x, return_relation=return_relation)
                # return qk, vv
            x = blk(x, register_blk == i,use_moe=(i==self.depth and use_moe))
        if use_moe:
            x = self.v_norm(x)
        else:
            x = self.norm(x)
        ...
        return x if not return_relation else (qk, vv, x)

    def forward_head(self, x, register_blk=-1, return_relation=False,use_moe=False):
        x = self.forward(x, register_blk, return_relation,use_moe)
...
class VisionTransformerEncoder(VisionTransformer, BaseEncoder):
    @classmethod
    def from_config(cls, cfg, from_pretrained=False,vision_width=768,num_heads=12,use_moe=False):
        ...
        if vit_type == "base":
            ...
            if from_pretrained:
                ...
                if vision_width==384:
                    url = "https://dl.fbaipublicfiles.com/deit/deit_small_patch16_224-cd65a155.pth" # deit small
    ...
    def forward_features(self, x, register_blk=-1,return_relation=False,use_moe=False):
        return super().forward(x, register_blk, return_relation=return_relation,use_moe=use_moe)
